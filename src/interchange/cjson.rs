...
pub fn canonicalize(jsn: &json::Value) -> Result<Vec<u8>> {
...
impl Value {
    fn write(&self, mut buf: &mut Vec<u8>) -> Result<()> {
        match self {
            &Value::Null => Ok(buf.extend(b"null")),
            &Value::Bool(true) => Ok(buf.extend(b"true")),
            &Value::Bool(false) => Ok(buf.extend(b"false")),
            &Value::Number(Number::I64(n)) => itoa::write(buf, n).map(|_| ()).map_err(|e| e.into()),
            &Value::Number(Number::U64(n)) => itoa::write(buf, n).map(|_| ()).map_err(|e| e.into()),
            &Value::String(ref s) => {
                // this mess is abusing serde_json to get json escaping
                let s = json::Value::String(s.clone());
                let s: Result<_> = json::to_string(&s).map_err(|e| e.into());
                let s = s?; // shut up, compiler
...
fn convert(jsn: &json::Value) -> Result<Value> {
    match jsn {
        ...
        &json::Value::Number(ref n) => {
            n.as_i64()
                ...
                .map(Value::Number)
                .ok_or_else(|| {
                    ErrorKind::IllegalArgument("Only i64 and u64 are supported in CJSON.".into())
                        .into()
                })
        }
